...
setlocal
cd /d "%~dp0"
set "TEMPCOPY=%TEMP%\Hel-Chobby_Update_%RANDOM%.bat"
...
>> temp.ps1 echo $web.DownloadFile('https://github.com/Helwor/Hel-Chobby/archive/main.zip', 'helchobby.zip')
>> temp.ps1 echo $shell = New-Object -ComObject Shell.Application
>> temp.ps1 echo $shell.Namespace((Get-Location).Path).CopyHere($shell.Namespace((Get-Item 'helchobby.zip').FullName).Items(), 16)
>> temp.ps1 echo Start-Sleep 2
>> temp.ps1 echo Remove-Item 'helchobby.zip'
...
if exist Hel-Chobby-main\.gitignore del Hel-Chobby-main\.gitignore
if not exist Hel-Chobby-main (
    echo Failed to extract package. Aborting.
    pause
    exit /b 1
)
> temp.ps1 echo if (Test-Path 'helchobby_manifest.txt') {
>> temp.ps1 echo $manifest = gc 'helchobby_manifest.txt'
>> temp.ps1 echo     Write-Host 'Checking removed files...'
>> temp.ps1 echo     $newDir = 'Hel-Chobby-main'
...
>> temp.ps1 echo     }
>> temp.ps1 echo }
>> temp.ps1 echo $d = (gi 'Hel-Chobby-main').FullName
>> temp.ps1 echo gci 'Hel-Chobby-main' -File -Recurse ^| ForEach-Object {
...
>> temp.ps1 echo }) ^| Out-File -Encoding utf8 'helchobby_manifest.txt'
powershell -ExecutionPolicy Bypass -File temp.ps1
del temp.ps1
for /l %%i in (1,1,3) do (
    if exist Hel-Chobby-main (
        timeout /t 1 /nobreak >nul
        rmdir /s /q Hel-Chobby-main 2>nul
    )
)
if exist Hel-Chobby-main (
    echo Warning: Could not delete Hel-Chobby-main folder. Please remove manually.
)
echo Done!
pause
exit /b