...
function ChatWindows:_NotifyTab(tabName, userName, chanName, nameMentioned, message, sound, popupDuration)
	local tabIsActive = tabName == self.currentTab
	-- Echo("Configuration:IsLobbyVisible() is ", Configuration:IsLobbyVisible())
	-- Echo("WG.LibLobby.lobby.visible is ", WG.LibLobby.lobby.visible)
	if chanName == 'Private' then
		lobby.lastPMUser = userName
	end
	if not tabIsActive or not WG.Chobby.Configuration:IsLobbyVisible() then
...
function widget:ConfigureLayout(command)
	Echo('CONFIGURE LAYOUT',command)
end

function widget:TextCommand(command)
	Echo('TEXT COMMAND',command)
end
