...
local function GetLobbyTabControls()
	...
	children[#children + 1], offset = AddCheckboxSetting(offset, "Notify on high elo 1v1 started", "notifyStartedHigh1v1", false)
	children[#children + 1], offset = AddCheckboxSetting(offset, "Notify any started match", "notifyAnyStarted", false)
...
local function ProcessSettingsSlider(data, offset, defaults, customSettingsSwitch)
	local Configuration = WG.Chobby.Configuration
	...
	local label = Label:New {
		...
		tooltip = data.desc,
		tooltip_format = '%s%%',
		tootipFunction = function(self)
			Spring.Echo('OK FUNC')
			return self.value
		end,
	}

	local function SetTrackbarValue(obj, newValue)
		-- newValue = string.gsub(newValue, "%%", "")
		-- newValue = tonumber(newValue)
		...
		if Configuration.settingsMenuValues[data.name] ~= newValue then
	...
	local slider = Trackbar:New {
		x = COMBO_X,
		y = offset,
		width  = COMBO_WIDTH,
		height = 30,
		value  = Configuration.settingsMenuValues[data.name] or defaults[data.name] or data.minValue,
		min    = data.minValue,
		max    = data.maxValue,
		step   = data.stepValue or 1,
		OnChange = {
			function(obj, value)
				if freezeSettings then
					Spring.Echo('FROZE')
	...
	settingsUpdateFunction[data.name] = function (newValue)
		SetTrackbarValue(slider, newValue)
	end

	return label, slider, offset + ITEM_OFFSET
end
...
local function PopulateTab(settingPresets, settingOptions, settingsDefault)
	...
	for i = 1, #settingOptions do
		...
		elseif data.isSliderSetting then
			label, list, offset = ProcessSettingsSlider(data, offset, settingsDefault, customSettingsSwitch)
