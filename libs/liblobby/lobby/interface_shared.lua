...
function Interface:_SendCommand(command, sendMessageCount)
    -- if command:find('Say') then
    --     Spring.Echo('interface_shared.lua send ','count',sendMessageCount,'msg',command)
    -- end
    ...
    -- local _, newcommand = self:_CallListeners("BeforeCommandSent", command:sub(1, #command-1))
    -- if newcommand then
    --     Echo('NEW COMMAND',newcommand)
    --  -- command = newcommand .. '\n'
    -- end
